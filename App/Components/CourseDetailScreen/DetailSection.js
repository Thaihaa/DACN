...
export default function DetailSection({course,enrollCourse,userEnrolledCourse}) {
  return (
    <View style={{padding:10,borderRadius:15,backgroundColor:Colors.WHITE}}>
      ...
      <Text style={{fontSize:24,fontFamily:'smbold',marginTop:10}}>{course.name}</Text>
      <View>
    ...
    <View>
      <Text style={{fontFamily:'smbold',fontSize:20}}>Description</Text>
    ...
    {userEnrolledCourse?.length==0?  (<TouchableOpacity onPress={()=>enrollCourse()} 
          style={{padding:15,backgroundColor:Colors.PRIMARY,borderRadius:10}}>
      ...
      <TouchableOpacity style={{padding:15,backgroundColor:Colors.SECONDARY,borderRadius:15}}>
        <Text style={{fontFamily:'outfit',color:Colors.WHITE,textAlign:'center',fontSize:17}}>Membership $1.99/Mon</Text>
      </TouchableOpacity>
    </View>
